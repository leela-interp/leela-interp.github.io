...
_LETTERS = "abcdefgh"
_LETTERS = {letter: i for i, letter in enumerate(_LETTERS)}
...
_idx_to_uci = idx_to_uci
_uci_to_idx = uci_to_idx
...
class LeelaBoard:
    ...
    def from_fen(
        ...
        uci: bool = False,
        history_synthesis: str | None = None,
    ):
        ...
        `history_synthesis` specified how to fill up the history if `moves` is not long
        enough. If "retro" (default), synthesize a random legal move history.
        If "repeat", just repeat the current board state (this is what Lc0 does by
        default, but is probably worse). If `None`, don't synthesize history (which
        can lead to egregious blunders sometimes).

        Moves are expected to be SAN, not UCI, unless uci=True.
        """
        board = chess.Board(fen)

        if history_synthesis == "retro":
            raise NotImplementedError("Retro history synthesis not implemented")
